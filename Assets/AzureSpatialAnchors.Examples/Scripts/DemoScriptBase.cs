...
namespace Microsoft.Azure.SpatialAnchors.Unity.Examples
{
    public abstract class DemoScriptBase : InputInteractionBase
    {
        ...
        protected DateTime dueDate2 = DateTime.MaxValue;
        protected readonly List<GameObject> allSpawnedObjects = new List<GameObject>();
        protected List<GameObject> allSpawnedObjects2 = new List<GameObject>();
        protected List<GameObject> allSpawnedObjects3 = new List<GameObject>();
        public Text Score;
        ...
        {
            base.Update();
            if (Points.path.Count!=0)
            {
                Vector3 tempPoint = Points.path[0];
                Points.path.RemoveAt(0);
                BlockGrass00();
                Quaternion rotation = Quaternion.AngleAxis(0, Vector3.up);
                SpawnNewAnchoredObject2(tempPoint, rotation);
                // AdvanceDemo();
            }
        }
        ...
        protected override void OnSelectObjectInteraction(Vector3 hitPoint, object target)
        {
            ...
            {
                Quaternion rotation = Quaternion.AngleAxis(0, Vector3.up);
                Debug.Log("get rotation: "+rotation);
                SpawnOrMoveCurrentAnchoredObject(hitPoint, rotation);
            }
        }
        ...
        protected virtual GameObject SpawnNewAnchoredObject(Vector3 worldPos, Quaternion worldRot)
        {
            ...
            // allSpawnedObjects2.Add(newGameObject);
            return newGameObject;
        }

        
        protected virtual void SpawnNewAnchoredObject3(List<Vector3>lists)
        {
            foreach (GameObject go in allSpawnedObjects3)
            {
                Destroy(go);
            }
            allSpawnedObjects3.Clear();
            BlockRed();
            for (int i = 0; i < lists.Count; i++)
            {
                // Create the prefab
                Quaternion rotation = Quaternion.AngleAxis(0, Vector3.up);
                GameObject newGameObject = GameObject.Instantiate(AnchoredObjectPrefab, lists[i], rotation);
                //
                // // Attach a cloud-native anchor behavior to help keep cloud
                // // and native anchors in sync.
                // newGameObject.AddComponent<CloudNativeAnchor>();
                ...
                // Return created object
                allSpawnedObjects3.Add(newGameObject);
            }
            
        }
        
        protected virtual void SpawnNewAnchoredObject2(Vector3 worldPos, Quaternion worldRot)
        {
            // Create the prefab
            GameObject newGameObject = GameObject.Instantiate(AnchoredObjectPrefab, worldPos, worldRot);
        ...
        /// Return Type: void
        /// Function: Give other subclass way to change prive
        ///           anchoredObjectPrefab value
        /// </summary>
        /// <param name="newObj"></param>
        public void setBlock(GameObject newObj)
        {
            anchoredObjectPrefab = newObj;
        }


        public void hiddenGrad()
        {
            foreach (GameObject go in allSpawnedObjects3)
            {
                go.SetActive(false);
        ...
        public void autoPlace()
        {
            ...
            // {
            //     Points.path.Add(new Vector3((float)Math.Round(Points.centerPoint.x+i,1), Points.centerPoint.y, Points.centerPoint.z));
            // }
            float offsetX = Points.centerPoint.x;
            float offsetY = Points.centerPoint.y;
            float offsetZ = Points.centerPoint.z;
            
            foreach (Vector3 point in Points.target)
            {
                Points.path.Add(new Vector3((float)Math.Round(point.x+offsetX,1), (float)Math.Round(point.y+offsetY,1), (float)Math.Round(point.z+offsetZ,1)));
            }
            Points.offset = new List<Vector3>(Points.path);
            foreach (GameObject go in allSpawnedObjects3)
            {
                go.SetActive(false);
            }
            ...
            NextBtn.SetActive(false);
            dueDate2 = DateTime.Now.AddSeconds(5);
            // dueDate3 = DateTime.Now.AddSeconds(7);
            // SpawnOrMoveCurrentAnchoredObject();
        }

        
        protected void TapBlock()
        {
            InputSwitch.ScreenEnable = true;
        ...
        protected void SwitchButton()
        {
            ...
            {
                InputSwitch.laserEnable = true;
                NextBtn.SetActive(true);
                dueDate3 = DateTime.Now;
            }
        ...
        protected void BlockRed()
        {
            GameObject newObj = Resources.Load("Red") as GameObject;
            setBlock(newObj);
        }
        
        protected void BlockCliff()
        {
            GameObject newObj = Resources.Load("Cliff") as GameObject;
            setBlock(newObj);
            BlockType.text = "Cliff";
            SwitchButton();
        }

        protected void BlockDirt()
        {
            GameObject newObj = Resources.Load("Dirt") as GameObject;
            setBlock(newObj);
            BlockType.text = "Dirt";
            SwitchButton();
        }

        protected void BlockGrass01()
        {
            GameObject newObj = Resources.Load("Grass_01") as GameObject;
            setBlock(newObj);
            BlockType.text = "Grass01";
            SwitchButton();
        }
        
        protected void BlockGrass00()
        {
            GameObject newObj = Resources.Load("Grass_01") as GameObject;
            setBlock(newObj);
        }

        protected void BlockGrass02()
        {
            GameObject newObj = Resources.Load("Grass_02") as GameObject;
            setBlock(newObj);
            BlockType.text = "Grass02";
            SwitchButton();
        }

        protected void BlockRock()
        {
            GameObject newObj = Resources.Load("Rock") as GameObject;
            setBlock(newObj);
            BlockType.text = "Rock";
            SwitchButton();
        }

        protected void BlockSand()
        {
            GameObject newObj = Resources.Load("Sand") as GameObject;
            setBlock(newObj);
            BlockType.text = "Sand";
            SwitchButton();
        }
